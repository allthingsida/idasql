...
TEST_F(TypesTableTest, TypesLiveTableExists) {
    auto result = query("SELECT name FROM sqlite_master WHERE type='table' AND name='types_live'");
    ASSERT_EQ(result.row_count(), 1);
    EXPECT_EQ(result.scalar(), "types_live");
}

TEST_F(TypesTableTest, TypesMembersLiveTableExists) {
    auto result = query("SELECT name FROM sqlite_master WHERE type='table' AND name='types_members_live'");
    ASSERT_EQ(result.row_count(), 1);
    EXPECT_EQ(result.scalar(), "types_members_live");
}

TEST_F(TypesTableTest, TypesEnumValuesLiveTableExists) {
    auto result = query("SELECT name FROM sqlite_master WHERE type='table' AND name='types_enum_values_live'");
    ASSERT_EQ(result.row_count(), 1);
    EXPECT_EQ(result.scalar(), "types_enum_values_live");
}

TEST_F(TypesTableTest, LiveTablesHaveSameColumnsAsReadOnly) {
    // types_live should have same columns as types
    auto types_cols = query("PRAGMA table_info(types)");
    auto types_live_cols = query("PRAGMA table_info(types_live)");
    EXPECT_EQ(types_cols.row_count(), types_live_cols.row_count())
        << "types_live should have same number of columns as types";

    // types_members_live should have same columns as types_members
    auto members_cols = query("PRAGMA table_info(types_members)");
    auto members_live_cols = query("PRAGMA table_info(types_members_live)");
    EXPECT_EQ(members_cols.row_count(), members_live_cols.row_count())
        << "types_members_live should have same number of columns as types_members";
}

TEST_F(TypesTableTest, LiveTablesHaveSameData) {
    // Live tables should have same data as read-only tables
    auto types_count = query("SELECT COUNT(*) FROM types");
    auto types_live_count = query("SELECT COUNT(*) FROM types_live");
    EXPECT_EQ(types_count.scalar_int(), types_live_count.scalar_int())
        << "types_live should have same row count as types";

    auto members_count = query("SELECT COUNT(*) FROM types_members");
    auto members_live_count = query("SELECT COUNT(*) FROM types_members_live");
    EXPECT_EQ(members_count.scalar_int(), members_live_count.scalar_int())
        << "types_members_live should have same row count as types_members";
}
