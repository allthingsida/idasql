...
NamePattern::NamePattern(const std::string& raw) {
...
bool NamePattern::matches(const std::string& value) const {
    if (!valid_) return false;
    return like_match(to_lower(value), pattern_);
}

std::string NamePattern::to_lower(const std::string& s) {
    std::string out;
    out.reserve(s.size());
    for (char c : s) {
...
bool NamePattern::has_wildcards(const std::string& s) {
    return s.find('%') != std::string::npos || s.find('_') != std::string::npos;
}

bool NamePattern::like_match(const std::string& text, const std::string& pattern) {
    size_t ti = 0;
    size_t pi = 0;
...
EntityGenerator::EntityGenerator(const std::string& pattern) : pattern_(pattern) {}

bool EntityGenerator::next() {
...
bool EntityGenerator::matches(const std::string& name) const {
    return pattern_.matches(name);
}

bool EntityGenerator::advance_current_source() {
...
bool EntityGenerator::advance_functions() {
...
bool EntityGenerator::advance_labels() {
...
bool EntityGenerator::advance_segments() {
...
bool EntityGenerator::advance_structs() { return advance_types_of_kind("struct", true, false, false); }
bool EntityGenerator::advance_unions()  { return advance_types_of_kind("union", false, true, false); }
...
GrepIterator::GrepIterator(const std::string& pattern)
    : generator_(pattern) {}

bool GrepIterator::next() {
    started_ = true;
...
bool GrepIterator::eof() const {
    return started_ && !valid_;
}

void GrepIterator::column(xsql::FunctionContext& ctx, int col) {
    if (!valid_) {
        ctx.result_null();
        return;
    }
...
int64_t GrepIterator::rowid() const {
    return rowid_;
}
