...
- `src/plugin/`: IDA plugin target (`idasql_plugin`); needs `IDASDK` and optional AI agent support via `IDASQL_WITH_AI_AGENT` + `external/libagents`.
- `src/common/`: shared plumbing for CLI/plugin (session handler, agent glue, embedded prompt header).
- `tests/`: GoogleTest suite, SQL fixtures in `tests/sql/`, sample database `tests/testdb.i64`.
- `external/libxsql` (+ `external/libagents` when AI agent enabled): bundled dependencies; init with `git submodule update --init external/libxsql external/libagents`.
- `prompts/`: agent prompt sources; regenerate the embedded header with `python scripts/embed_prompt.py prompts/idasql_agent.md src/common/idasql_agent_prompt.hpp`.
- `scripts/`: helper utilities (prompt embedding, misc automation); build outputs live under `build/`, `build/cli`, `build/plugin`, `build/tests`.
- `examples/`, `experiments/`: demos and scratch/prototyping code (may require IDA/Hex-Rays).

## Build, Test, and Development Commands

Prereqs: CMake (>= 3.20; tests need >= 3.27), a C++17 toolchain, IDA SDK (`IDASDK` includes `ida-cmake`), and `xsql::xsql` (`../libxsql` or `-Dxsql_DIR=...`). Init submodules if missing: `git submodule update --init external/libxsql external/libagents`.
...
- Plugin: `cmake -S src/plugin -B build/plugin [-DIDASQL_WITH_AI_AGENT=ON]` then `cmake --build build/plugin --config Release` (requires `%IDASDK%`, Hex-Rays optional; AI agent flag pulls in `external/libagents`).
- Prompt embedding (needed after editing `prompts/idasql_agent.md`): `python scripts/embed_prompt.py prompts/idasql_agent.md src/common/idasql_agent_prompt.hpp`
